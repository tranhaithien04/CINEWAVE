...
export function MovieCard({ movie, priority = false }: { movie: Movie; priority?: boolean }) {
  ...
  return (
    <motion.div
      ...
      className="h-full"
    >
      <article className="group relative flex h-full flex-col overflow-hidden rounded-2xl border border-white/10 bg-cinema-900/70 shadow-xl backdrop-blur-md transition-all duration-300 hover:border-cyan-500/50 hover:shadow-2xl hover:shadow-cyan-500/15">
        <Link href={paths.movie(movie.slug)} className="relative aspect-[2/3] w-full overflow-hidden bg-cinema-800">
          ...
          <span className="absolute inset-0 z-10 flex items-center justify-center opacity-0 transition-all duration-300 group-hover:opacity-100">
