  ...
  function scan() {
    ...
    window.setTimeout(() => {
      void (async () => {
        ...
        if (user) {
          try {
            await submitAgeVerification({ passed, rating, movieSlug });
          } catch (error) {
            toast.error(error instanceof Error ? error.message : "Không ghi nhận được xác minh tuổi");
          }
        }
        setStep(passed ? "success" : "fail");
        if (passed) {
          toast.success("Đủ tuổi xem suất này. Không lưu ảnh CCCD.");
          onPassed();
        } else {
          toast.error("Không nhận diện được giấy tờ. Thử ảnh rõ hơn.");
        }
      })();
    }, 1400);
  }
      ...
      <DialogContent className="rounded-2xl border-white/10 sm:max-w-md">
        ...
        {step === "scanning" ? (
          <div className="relative flex aspect-[16/10] w-full flex-col items-center justify-center overflow-hidden rounded-2xl border-2 border-cyan-500/50 bg-black/80 text-center">
            ...
            <p className="text-xs text-gray-400">Không hiển thị họ tên hay số định danh.</p>
          </div>
        ) : null}
        ...
        {step === "fail" ? (
          <div className="flex flex-col items-center gap-2 rounded-2xl border border-destructive/30 bg-destructive/10 p-6 text-center">
            ...
            <p className="text-xs text-muted-foreground">Chụp lại mặt trước, đủ sáng, không chói.</p>
          </div>
        ) : null}
        ...
        <DialogFooter className="gap-2 sm:justify-end">
          ...
          {step === "preview" ? (
            <Button onClick={scan} className="rounded-xl">
              Nhận diện
            </Button>
          ) : null}
          {step === "fail" ? (
